...
def test_subject_dashboard_project_cards_offer_project_delete_action() -> None:
    ...
    assert "默认项目与学科根绑定，不能单独删除" not in source
    assert "canDeleteMaterial" not in source
    assert "title={deleteMaterialBlockedReason}" not in action_row_source
    assert "disabled={deleteMaterialM.isPending}" in action_row_source
