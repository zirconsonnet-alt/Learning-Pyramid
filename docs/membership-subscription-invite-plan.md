...
- 已将月会员价格调整为 `20 元`
- 已新增 `考研套餐`：`planId=graduate_exam`，仅支持在每年 `11 月 21 日前` 购买；前端展示为约 `15 元/月`，实际按购买当天到同年 `12 月 21 日` 的自然日逐日计价，每日 `0.5 元`
...
- 月会员标准价：`20 元`
- 考研套餐：`11 月 21 日前`可购买，前端展示为约 `15 元/月`，实际价格为购买当天到同年 `12 月 21 日` 的天数乘以每日 `0.5 元`
...
- 月会员按 `30 天` 发放；考研套餐按购买当天到同年 `12 月 21 日` 的天数发放
- 邀请折扣券有效期建议 `30 天`
...
- 普通用户注册时必须携带 `inviteCode`；仅部署时显式配置的引导超管邮箱可留空
- 创建用户
- 按 `public_uid` 查邀请人并校验 `inviteCode`
- 禁止绑定自己
- 写入 `invite_bindings`
