...
- 考研套餐：每年 `11 月 21 日前`可购买，前端展示为约 `15 元/月`，实际按购买当天到同年 `12 月 21 日` 逐日计价，每日 `0.5 元`
...
- 如果准备公网自由开放注册，`PLM_SMTP_HOST` / `PLM_SMTP_FROM_EMAIL` / `PLM_PUBLIC_ORIGIN` / `PLM_ALTCHA_HMAC_SECRET` 已配好
