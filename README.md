...
LearningPyramid 是一个本地优先的学习系统，用于围绕你自己的视频或音频材料构建复述点、复习链，并在存在时直接利用视频同目录同名字幕文件作为播放器字幕与 AI 文本上下文。工作台视频助手还会在提问时附带当前复述点内容和当前视频帧；桌宠现在提供“复制文本”和“复制图片”两个按钮，把当前上下文和当前帧直接放进剪贴板，且不会调用 LLM。
