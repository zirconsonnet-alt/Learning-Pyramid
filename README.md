...
LearningPyramid 是一个本地优先的学习系统，用于围绕你自己的视频或音频材料构建复述点、复习链，并在存在时直接利用视频同目录同名字幕文件作为播放器字幕与 AI 文本上下文。工作台视频助手还会在提问时附带当前复述点内容和当前视频帧；桌宠可在获取上下文模式下导出包含文字上下文和内嵌当前帧的 HTML 包，且不会调用 LLM。
