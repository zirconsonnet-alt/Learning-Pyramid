...
function createMarkdownComponents(textClassName?: string): Components {
  const bodyTextClassName = cn("text-[15px] leading-7 text-current", textClassName)

  return {
    p({ children }) {
      return <p className={cn("break-words", bodyTextClassName)}>{children}</p>
    },
    a({ href, children }) {
      return (
        <a
    ...
    h1({ children }) {
      return <h1 className="text-2xl font-semibold leading-9 text-current">{children}</h1>
    },
    h2({ children }) {
      return <h2 className="text-xl font-semibold leading-8 text-current">{children}</h2>
    },
    h3({ children }) {
      return <h3 className="text-lg font-semibold leading-8 text-current">{children}</h3>
    },
    h4({ children }) {
      return <h4 className="text-base font-semibold leading-7 text-current">{children}</h4>
    },
    blockquote({ children }) {
      return <blockquote className={cn("border-l-2 border-[color:var(--theme-soft-border)] pl-4", bodyTextClassName)}>{children}</blockquote>
    },
    ul({ children }) {
      return <ul className={cn("list-disc space-y-2 pl-6", bodyTextClassName)}>{children}</ul>
    },
    ol({ children }) {
      return <ol className={cn("list-decimal space-y-2 pl-6", bodyTextClassName)}>{children}</ol>
    },
    li({ children }) {
    ...
    table({ children }) {
      return (
        <div className="overflow-x-auto rounded-[1.25rem] border border-[color:var(--theme-soft-border)]">
          <table className="min-w-full border-collapse text-left text-sm leading-6 text-current">{children}</table>
        </div>
      )
    },
    th({ children }) {
      return (
        <th className="border-b border-[color:var(--theme-soft-border)] bg-[color:var(--theme-soft-bg)] px-4 py-3 font-semibold text-current">
          {children}
        </th>
      )
    },
...
export function MarkdownRichText({
  text,
  className,
  textClassName,
}: {
  ...
  const components = useMemo(() => createMarkdownComponents(textClassName), [textClassName])
