...
export function ProjectSettingsPage() {
  ...
  const loadedTemplateItems = useMemo(
    () => toTemplateEditorItems(effectiveLayerConfig.reviewChainTemplate),
  ...
  const [cfgKNode, setCfgKNode] = useState("")
  const [cfgKPoint, setCfgKPoint] = useState("")
  const [cfgTemplateItems, setCfgTemplateItems] = useState<TemplateEditorItem[]>([])
  ...
  useEffect(() => {
    setCfgErr(null)
    setCfgKNode(String(effectiveLayerConfig.aggregationKNode))
    setCfgKPoint(String(effectiveLayerConfig.aggregationKPoint))
    setCfgTemplateItems(loadedTemplateItems)
  }, [effectiveLayerConfig.aggregationKNode, effectiveLayerConfig.aggregationKPoint, layerConfigVersion, loadedTemplateItems])

  useEffect(() => {
    const external = projectConfigQ.data?.externalServices
    setAsrModel(external?.asr?.model ?? "")
  }, [projectConfigQ.data?.externalServices])
  ...
  useEffect(() => {
    setRemapTargets((prev) => {
      ...
      for (const instance of missingInstances) {
        const existing = prev[instance.instanceId]
        if (existing && presentInstances.some((item) => item.instanceId === existing)) {
          next[instance.instanceId] = existing
          continue
        }
        const suggested = suggestTargetInstance(instance, presentInstances)
        if (suggested) next[instance.instanceId] = suggested
      }
      return next
    })
  }, [missingInstances, presentInstances])
  ...
  async function onSaveLayerConfig() {
    ...
    for (const item of cfgTemplateItems) {
      ...
      items.push(n === 1 ? { kind: "REVIEW_TASK" } : { kind: "REVIEW_TASK", count: n })
    }
    ...
    await setLayerConfigM.mutateAsync({
      ...
      kNode,
      kPoint,
      reviewChainTemplate: items,
    })
  }

  async function onSaveExternalServices() {
    const trimmedAsrModel = asrModel.trim()
  ...
  async function onRemapMissingInstance(fromInstanceId: string) {
    const toInstanceId = remapTargets[fromInstanceId]
                ...
                <div className="mt-3 space-y-3">
                  <div>
                    ...
                    <Input
                      ...
                      disabled={setExternalServicesM.isPending}
                    />
                  </div>
                </div>
              </div>

              <Button onClick={() => void onSaveExternalServices()} disabled={setExternalServicesM.isPending || !pid}>
                {setExternalServicesM.isPending ? "保存中..." : "保存转写设置"}
              </Button>

              {projectConfigQ.isLoading ? <p className="text-sm text-muted-foreground">加载当前转写设置中...</p> : null}
              {projectConfigQ.error ? <p className="text-sm text-destructive">{formatApiError(projectConfigQ.error)}</p> : null}
              {setExternalServicesM.error ? <p className="text-sm text-destructive">{formatApiError(setExternalServicesM.error)}</p> : null}
            </CardContent>
          </Card>
        </div>
            ...
            <div className="grid gap-3">
              <div>
                ...
                <select
                  ...
                  value={String(effectiveConfigLayerIndex)}
                  onChange={(e) => setConfigLayerIndex(Number(e.target.value))}
                  disabled={layerIndexes.length === 0 || setLayerConfigM.isPending}
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input id="kNode" value={cfgKNode} onChange={(e) => setCfgKNode(e.target.value)} disabled={setLayerConfigM.isPending} />
                </div>
                <div>
                  <Label htmlFor="kPoint">复述点阈值</Label>
                  <Input id="kPoint" value={cfgKPoint} onChange={(e) => setCfgKPoint(e.target.value)} disabled={setLayerConfigM.isPending} />
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    disabled={setLayerConfigM.isPending}
                  >
                    追加收敛
                  </Button>
                  <Button
              ...
              <Button onClick={() => void onSaveLayerConfig()} disabled={setLayerConfigM.isPending || !pid}>
                {setLayerConfigM.isPending ? "保存中..." : "保存配置"}
              </Button>
              ...
              {setLayerConfigM.error ? <p className="text-sm text-destructive">{formatApiError(setLayerConfigM.error)}</p> : null}
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="space-y-3 text-sm">
          ...
          <div className="space-y-3">
            {missingInstances.map((instance, index) => {
              ...
              const targetInstanceId = remapTargets[instance.instanceId] ?? ""
