...
test("instance detail removes opaque ids from the summary card", async ({ page }) => {
  ...
  await expect(summaryCard.getByText("关联节点")).toBeVisible()
  await expect(summaryCard.getByRole("link", { name: "查看对象节点" })).toHaveAttribute("href", projectPath(`/learning-object-nodes/${learningObjectNode.nodeId}`))
  await expect(page.getByRole("button", { name: "查看对象节点" })).toHaveCount(0)
  await expect(page.getByTestId("video-pane-card")).toHaveAttribute("data-surface", "detail")
  await expect(page.getByRole("heading", { name: "复述点列表" })).toHaveCount(0)
